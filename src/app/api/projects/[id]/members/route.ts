...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const projectMembership = query('SELECT * FROM project_members WHERE project_id = ? AND user_id = ?', [id, currentUserId]);
    if (projectMembership.length === 0) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    }

    const teamMembership = query('SELECT * FROM team_members WHERE team_id = ? AND user_id = ?', [project[0].team_id as string, userId]);
    if (teamMembership.length === 0) {
      return NextResponse.json({ error: 'User is not a team member' }, { status: 400 });
    }

    const existing = query('SELECT * FROM project_members WHERE project_id = ? AND user_id = ?', [id, userId]);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await getDb();
    const ownership = query('SELECT * FROM projects WHERE id = ? AND owner_id = ?', [id, currentUserId]);
    ...
    console.error('PATCH /api/projects/[id]/members error:', err);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (userId === currentUserId) {
      const isOwner = query('SELECT * FROM projects WHERE id = ? AND owner_id = ?', [id, currentUserId]);
      if (isOwner.length > 0) {
        return NextResponse.json({ error: 'Owner cannot leave project' }, { status: 400 });
      }
    }
