...
export default function AIActionMenu({ isOpen, onClose, onSelect, blockType, position }: AIActionMenuProps) {
  ...
  return (
    <div
      ref={menuRef}
      className="fixed z-50 w-60 bg-zinc-900/95 backdrop-blur-xl border border-zinc-700/50 rounded-xl shadow-2xl shadow-black/50 overflow-hidden animate-scale-in"
      style={{ top: position.top, left: position.left }}
    >
      <div className="px-3 py-2 border-b border-zinc-800/60">
        <span className="text-[11px] text-zinc-500 font-mono uppercase tracking-wider">AI Actions</span>
      </div>
      {actions.map((action) => (
        <button
          key={action.id}
        ...
        >
          <span className="p-1 rounded text-zinc-500 group-hover:text-blue-400 group-hover:bg-blue-500/10 transition-colors">
            {iconMap[action.icon]}
          </span>
          <div>
            <div className="text-sm text-zinc-200">{action.label}</div>
            <div className="text-[11px] text-zinc-500">{action.description}</div>
          </div>
        </button>
      ))}
    </div>
