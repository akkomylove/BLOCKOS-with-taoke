...
export default function CreateProjectModal({ isOpen, onClose, teamId }: CreateProjectModalProps) {
  const createProject = useCollaborationStore((state) => state.createProject);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await createProject(teamId, name.trim(), description.trim(), selectedIcon, selectedColor);
      onClose();
      setName('');
      setDescription('');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-scale-in">
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-zinc-700">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-zinc-100">新建项目</h2>
          <button
            onClick={onClose}
