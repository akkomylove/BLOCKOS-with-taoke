    ...
    <div className="fixed right-0 top-14 bottom-0 w-96 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl border-l border-gray-200 dark:border-zinc-800/50 z-40 overflow-hidden flex flex-col animate-slide-up shadow-2xl shadow-black/30">
      <div className="flex items-center justify-between px-4 py-3.5 border-b border-gray-200 dark:border-zinc-800/60">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-semibold text-gray-900 dark:text-zinc-100">关系视图</h3>
        </div>
        ...
        {activeTab === 'links' && (
          <div className="space-y-4 pt-2">
            <div className="grid grid-cols-2 gap-2">
                <div className="bg-gray-100/40 dark:bg-zinc-800/40 border border-gray-200/40 dark:border-zinc-800/40 rounded-xl px-3 py-3">
                  <div className="text-lg font-semibold text-gray-800 dark:text-zinc-200">{stats.totalLinks}</div>
                  <div className="text-[10px] text-gray-400 dark:text-zinc-500 uppercase tracking-wider mt-0.5">正向链接</div>
                </div>
                <div className="bg-gray-100/40 dark:bg-zinc-800/40 border border-gray-200/40 dark:border-zinc-800/40 rounded-xl px-3 py-3">
                  <div className="text-lg font-semibold text-gray-800 dark:text-zinc-200">{stats.totalBacklinks}</div>
                  <div className="text-[10px] text-gray-400 dark:text-zinc-500 uppercase tracking-wider mt-0.5">反向链接</div>
                </div>
              </div>

              <div>
                <h4 className="text-[11px] font-medium text-gray-500 dark:text-zinc-400 mb-3 flex items-center gap-1.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-400" />
                  正向链接
                </h4>
                {links.length === 0 ? (
                  <div className="text-xs text-gray-400 dark:text-zinc-600 py-6 px-3 bg-gray-100/30 dark:bg-zinc-800/30 rounded-xl text-center leading-relaxed">
                  ...
                  <div className="space-y-2">
                    {links.map((link, i) => {
                      ...
                      return (
                        <div key={i} className="bg-gray-100/40 dark:bg-zinc-800/40 rounded-xl border border-gray-200/40 dark:border-zinc-800/40 overflow-hidden hover:border-blue-500/20 transition-all">
                          <div className="px-3 py-2.5 flex items-center gap-2.5">
                            ...
                            <span
                              className="text-gray-700 dark:text-zinc-300 truncate flex-1 text-xs cursor-pointer hover:text-blue-400 transition-colors"
                              onClick={() => handleSelectBlock(link.from.id)}
                            >
                              {link.from.title || link.from.content.substring(0, 30) || '未命名 Block'}
                            </span>
                          </div>
                          <div className="px-3 py-1.5 flex items-center justify-center bg-gray-100/20 dark:bg-zinc-800/20">
                            <ArrowRight className="w-3.5 h-3.5 text-blue-500/50" />
                          </div>
                          <div className="px-3 py-2.5 flex items-center gap-2.5 border-t border-gray-200/30 dark:border-zinc-800/30">
              ...
              <div>
                ...
                {backLinks.length === 0 ? (
                  <div className="text-xs text-gray-400 dark:text-zinc-600 py-6 px-3 bg-gray-100/30 dark:bg-zinc-800/30 rounded-xl text-center">暂无反向链接</div>
                ) : (
                  <div className="space-y-2">
                    {backLinks.map((item) => (
                      <div key={item.block.id} className="bg-gray-100/40 dark:bg-zinc-800/40 rounded-xl border border-gray-200/40 dark:border-zinc-800/40 p-3.5 hover:border-purple-500/20 transition-all">
                        <div
                          className="text-xs text-gray-700 dark:text-zinc-300 mb-2.5 cursor-pointer hover:text-purple-400 transition-colors font-medium"
                          onClick={() => handleSelectBlock(item.block.id)}
                        >
                          {item.block.title || item.block.content.substring(0, 35) || '未命名 Block'}
                        </div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <ArrowLeft className="w-3 h-3 text-gray-400 dark:text-zinc-600 shrink-0" />
                          {item.referrers.map((ref) => (
                            <span
                              key={ref.id}
                              className="text-[10px] text-gray-500 dark:text-zinc-400 bg-gray-200/60 dark:bg-zinc-700/60 px-2 py-0.5 rounded-md cursor-pointer hover:bg-gray-300 dark:hover:bg-zinc-600 hover:text-gray-800 dark:hover:text-zinc-200 transition-colors"
          ...
          {activeTab === 'hierarchy' && (
            <div className="space-y-3 pt-2">
              <div className="bg-gray-100/40 dark:bg-zinc-800/40 border border-gray-200/40 dark:border-zinc-800/40 rounded-xl px-3 py-3">
                <div className="text-lg font-semibold text-gray-800 dark:text-zinc-200">{hierarchy.length}</div>
                <div className="text-[10px] text-gray-400 dark:text-zinc-500 uppercase tracking-wider mt-0.5">层级节点</div>
              </div>
                ...
                <div className="space-y-1">
                  {hierarchy.map(({ block, depth }) => {
                    ...
                    return (
                      <div
                        key={block.id}
                        className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-gray-100/60 dark:hover:bg-zinc-800/60 transition-all cursor-pointer border border-transparent hover:border-gray-300/40 dark:hover:border-zinc-700/40 group"
                        style={{ marginLeft: `${depth * 14}px` }}
                        onClick={() => handleSelectBlock(block.id)}
                      >
                        {depth > 0 && (
                          <div className="w-3 h-3 flex items-center justify-center shrink-0">
                            <div className="w-1 h-1 rounded-full bg-gray-400 dark:bg-zinc-600" />
                          </div>
                        )}
                        <div className={`w-6 h-6 rounded-lg ${colors.bg} flex items-center justify-center shrink-0`}>
                          <Box className={`w-3 h-3 ${colors.text}`} />
                        </div>
                        <span className="text-xs text-gray-700 dark:text-zinc-300 truncate flex-1 group-hover:text-gray-900 dark:group-hover:text-zinc-100 transition-colors">
                          {block.title || block.content.substring(0, 25) || '未命名 Block'}
                        </span>
                        {blocks.some((b) => b.parentId === block.id) && (
                          <span className="text-[9px] text-gray-400 dark:text-zinc-600 font-mono bg-gray-100/60 dark:bg-zinc-800/60 px-1.5 py-0.5 rounded">
          ...
          {activeTab === 'graph' && (
            <div className="space-y-4 pt-2">
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-gray-100/40 dark:bg-zinc-800/40 border border-gray-200/40 dark:border-zinc-800/40 rounded-xl px-3 py-3">
                  <div className="text-lg font-semibold text-gray-800 dark:text-zinc-200">{stats.totalBlocks}</div>
                  <div className="text-[10px] text-gray-400 dark:text-zinc-500 uppercase tracking-wider mt-0.5">Block 总数</div>
                </div>
                <div className="bg-gray-100/40 dark:bg-zinc-800/40 border border-gray-200/40 dark:border-zinc-800/40 rounded-xl px-3 py-3">
                  <div className="text-lg font-semibold text-gray-800 dark:text-zinc-200">{stats.totalLinks + stats.totalBacklinks}</div>
                  <div className="text-[10px] text-gray-400 dark:text-zinc-500 uppercase tracking-wider mt-0.5">连接总数</div>
                </div>
                <div className="bg-gray-100/40 dark:bg-zinc-800/40 border border-gray-200/40 dark:border-zinc-800/40 rounded-xl px-3 py-3">
                  <div className="text-lg font-semibold text-gray-800 dark:text-zinc-200">{blocks.filter((b) => b.parentId).length}</div>
                  <div className="text-[10px] text-gray-400 dark:text-zinc-500 uppercase tracking-wider mt-0.5">子 Block</div>
                </div>
                <div className="bg-gray-100/40 dark:bg-zinc-800/40 border border-gray-200/40 dark:border-zinc-800/40 rounded-xl px-3 py-3">
                  <div className="text-lg font-semibold text-gray-800 dark:text-zinc-200">{stats.orphanedBlocks}</div>
                  <div className="text-[10px] text-gray-400 dark:text-zinc-500 uppercase tracking-wider mt-0.5">孤立 Block</div>
                </div>
              </div>

              <div className="bg-gray-100/30 dark:bg-zinc-800/30 border border-gray-200/40 dark:border-zinc-800/40 rounded-xl p-4">
                <h4 className="text-[11px] font-medium text-gray-500 dark:text-zinc-400 mb-3 flex items-center gap-1.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-400" />
                  类型分布
                </h4>
                  ...
                  {Array.from(new Set(blocks.map((b) => b.type))).map((type) => {
                    ...
                    return (
                      <div key={type} className="flex items-center gap-2.5">
                        ...
                        <div className="flex-1 h-2 bg-gray-200/50 dark:bg-zinc-700/50 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all duration-500 ${colors.dot}`}
                            style={{ width: `${Math.max(percentage, 4)}%` }}
                          />
                        </div>
                        <span className="text-[10px] text-gray-500 dark:text-zinc-400 font-mono w-6 text-right">{count}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="bg-gray-100/30 dark:bg-zinc-800/30 border border-gray-200/40 dark:border-zinc-800/40 rounded-xl p-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2.5 bg-gray-200/50 dark:bg-zinc-700/50 rounded-full overflow-hidden">
                    <div
                  ...
                  <span className="text-xs text-gray-700 dark:text-zinc-300 font-mono font-semibold">
                    {stats.totalBlocks > 0 ? ((stats.totalLinks + stats.totalBacklinks) / stats.totalBlocks).toFixed(1) : '0.0'}x
                  </span>
                </div>
                <p className="text-[10px] text-gray-400 dark:text-zinc-600 mt-2">每个 Block 平均拥有的连接数</p>
              </div>
            </div>
          )}
      </div>
